...
/*   Updated: 2021/11/07 12:41:45 by rafernan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./libps.h"

// Not cheking for input bigger then int
t_list	*ps_lstatoi(char **list, int size)
{
	...
	while (++i < size)
	{
		...
		if (!tmp->next || ((long)(tmp->next->content) == 0 && **(list + i) != '0')) // Add a verify param function here
			ps_exit(&stack, 1);
		tmp = (tmp->next);
	}
	return (stack);
